...
public class HumanPyramid {
     ...
     */
    /**
     * Recursively calculates the total weight supported by a person in the pyramid.
     * Each person supports their own weight plus half the weight supported by each person above them.
     *
     * @param pyramid List of lists representing the pyramid weights.
     * @param row Row index of the person (0-indexed from top).
     * @param col Column index of the person (0-indexed from left).
     * @return The total weight supported by the person at (row, col).
     */
    public float totalWeightSupported(List<List<Double>> pyramid, int row, int col) {
        // Base case: top row
        if (row == 0) {
            return pyramid.get(0).get(0).floatValue();
        }
        float self = pyramid.get(row).get(col).floatValue();
        float aboveLeft = (col > 0) ? totalWeightSupported(pyramid, row - 1, col - 1) / 2 : 0;
        float aboveRight = (col < pyramid.get(row - 1).size()) ? totalWeightSupported(pyramid, row - 1, col) / 2 : 0;
        return self + aboveLeft + aboveRight;
    }
